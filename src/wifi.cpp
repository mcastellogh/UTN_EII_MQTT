//=======================================================================
// File:                wifi.cpp
// Author:              Marcelo Castello (https://github.com/mcastellogh)
// Licence:             GPLV3+
// Version:             1.0.0
// Date:                Julio 2024
// Info:                Part of project for Electrónica II
//=======================================================================

#include "wifi.h"
//--Local variables
uint32_t _wifi_crono;
uint8_t _wifi_state = WIFI_RECONNECT;
uint8_t _wifi_last_state = WIFI_DISCONNECTED;
uint8_t _try_wifi_reconn=1;
uint8_t wifi_led_state = LED_OFF;
bool _flag_wifi_reconnected=0;
uint32_t _ton;
uint32_t _toff;
bool _ap_state;

//--Extern variables
extern Config config;

//--Init and connect wifi network
bool wifi_init(WiFiMode_t mode){    
    WiFi.persistent(false);
    WiFi.mode(mode);
    WiFi.setAutoConnect(false);
    WiFi.setAutoReconnect(false);
    WiFi.disconnect();

    //--WiFi mode
    switch (mode){
        case WIFI_AP_STA:
            _APconnect();
            _wifi_state=_connect();
            break;
        case WIFI_AP:
            _APconnect();
            break;
        case WIFI_STA:
            _wifi_state=_connect();
            break;
        default:
            break;
    }
    if (_wifi_state==WIFI_CONNECTED){
        return true;
    }else{
        return false;
    }
}

//--FMS for maintain wifi connection. Return state of connection (T/F)
bool wifi_loop(void){
  switch (_wifi_state){
    case WIFI_CONNECTED:
      if (_get_wifi_status()!=WIFI_CONNECTED){
        _wifi_state=WIFI_DISCONNECTED;
      }
      break;

    case WIFI_RECONNECT:
        Serial.print(F("WiFi reconnect "));
        Serial.println(_try_wifi_reconn);
        _try_wifi_reconn++;
        WiFi.begin(config.ssid,config.ssid_pass);
        _wifi_crono = millis();
        _wifi_state=WIFI_WAITFORCONNECT;
        break;

    case WIFI_DISCONNECTED:
        if(_get_wifi_status()!=WIFI_CONNECTED){
          WiFi.disconnect();
          _wifi_crono = millis();
          _wifi_state=WIFI_RECONNECT;
          _flag_wifi_reconnected=1;
        }else{
          _wifi_state=WIFI_CONNECTED;
        }
        break;

    case WIFI_WAITTORECONNECT:
      if (millis() - _wifi_crono >= RECONN_TIMEOUT){
        _wifi_state=WIFI_RECONNECT;
      }
      break;

    case WIFI_WAITFORCONNECT:
      if(_get_wifi_status()!=WIFI_CONNECTED){
        if (millis() - _wifi_crono >= WAITFORCONNECT_TIMEOUT){
          _wifi_crono = millis();
          _wifi_state=WIFI_WAITTORECONNECT;
        }
      }else{
         if(_flag_wifi_reconnected){
          Serial.println(F("WiFi reconnected!!"));
          _try_wifi_reconn=0;
          _flag_wifi_reconnected=0;
        }
        _wifi_state=WIFI_CONNECTED;
      }
      break;

    default:
      break;
  }
  _FSM_state_led(_wifi_state, WIFI_LED_PIN);
  
  if (_wifi_state==WIFI_CONNECTED){
    return true;
  }else{
    return false;
  }
}




//--Connect to wifi network. Return state of connection (T/F)
bool _connect(void){
  Serial.printf("Connecting to %s with %s password", config.ssid, config.ssid_pass);
  //WiFi.begin("MAC_E","wpaadministrator08");
  WiFi.begin(config.ssid, config.ssid_pass);
  _wifi_crono = millis();
  while (WiFi.status()!=WL_CONNECTED){
    Serial.print(F("."));
    _FSM_state_led(WIFI_WAITFORCONNECT, WIFI_LED_PIN );
    delay(100);
    if (millis() - _wifi_crono >= WAITFORCONNECT_TIMEOUT){
      break;
    }
  }
  Serial.println("");
  _wifi_state=_get_wifi_status();
  _wifi_crono = millis();
  _FSM_state_led(_wifi_state, WIFI_LED_PIN );
  if (_wifi_state==WIFI_CONNECTED){
    Serial.printf("Connected to %s with IP: ", config.ssid);
    Serial.println(WiFi.localIP());
    return true;
  }else{
      Serial.println("Not connected!");
    return false;
  }
}

//--State machine for wifi led
void _FSM_state_led(uint8_t _wifi_state, uint8_t led){
  if(_wifi_last_state!=_wifi_state){
    switch (_wifi_state){
      case WIFI_CONNECTED:      
        _ton=10000;
        _toff=0;
        break;
      case WIFI_DISCONNECTED:
        _ton=0;
        _toff=10000;
        break;
      case WIFI_WAITTORECONNECT:
        _ton=30;
        _toff=30;
        break;
      case WIFI_WAITFORCONNECT:
        _ton=200;
        _toff=200;
        break;        
      default:
        break;
    }
    _wifi_last_state=_wifi_state;
  }
  led_blink_FSM(_ton, _toff, led);
}


//--Get the wifi state for state machine. Return state (see FSM_WIFI_STATES enum in wifi.h)
uint8_t _get_wifi_status(void){
  switch (WiFi.status()){
    case WL_CONNECTED:
      if(WiFi.localIP()[3]==0){ //check if have ip number (Fail terterbio01)
        return WIFI_DISCONNECTED;
      }
      return WIFI_CONNECTED;
      break;
    case WL_IDLE_STATUS:
      return WIFI_WAITFORCONNECT;
      break;
    case WL_NO_SSID_AVAIL:
    case WL_CONNECT_FAILED:
    case WL_CONNECTION_LOST:
    case WL_DISCONNECTED: 
      return WIFI_DISCONNECTED;
      break;
    default:
      return WIFI_DISCONNECTED;
      break;
  }
}

//--Access Point init 
void _APconnect(void){
  _ap_state=WiFi.softAP(config.ap_ssid, config.ap_pass);
  if (_ap_state){
    Serial.print("AP ready with IP: ");
    Serial.println(WiFi.softAPIP());
  }else{
    Serial.println("AP init faill");  
  }
}